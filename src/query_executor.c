...
void BuildGraph(QueryGraph *graph, Vector *entities) {
    ...
    for(int i = 0; i < Vector_Size(entities); i++) {
        ...
        /* No duplicates. */
        if(QueryGraph_GetNodeByAlias(graph, entity->alias) != NULL) continue;
    ...
    for(int i = 0; i < Vector_Size(entities); i++) {
        ...
        /* No duplicates. */
        if(QueryGraph_GetEdgeByAlias(graph, entity->alias) != NULL) continue;
