...
(
	...
	if(e->prop_count > 0) {
		// clone properties
		clone->properties = rm_malloc(clone->prop_count * sizeof(EntityProperty));
		for(int i = 0; i < clone->prop_count; i ++) {
			clone->properties[i].id = e->properties[i].id;
			clone->properties[i].value = SI_CloneValue(e->properties[i].value);
		}
	}
}
