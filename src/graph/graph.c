...
(
	...
	GrB_Index i;            // iterated entry row idx
	GrB_Index j;            // iterated entry col idx
	GrB_Info info;          // GraphBLAS return code
	GrB_Matrix tmp;         // temp matrix collecting future entries to delete
	GrB_Index nrows;        // lbls matrix row count
	GrB_Index ncols;        // lbls matrix col count
	GxB_Iterator iter;      // temp matrix iterator
	RG_MatrixTupleIter it;  // lbls matrix iterator
	...
	ASSERT(info == GrB_SUCCESS);
	info = GrB_Matrix_new(&tmp, GrB_BOOL, nrows, ncols);
	ASSERT(info == GrB_SUCCESS);

	// attach iterator to lbls matrix
	RG_MatrixTupleIter_attach(&it, lbls);

	// iterate each lbls row coresponding to a deleted node
	...
	for(uint i = 0; i < count; i++) {
		...
		while(RG_MatrixTupleIter_next_BOOL(&it, NULL, &j, NULL) == GrB_SUCCESS) {
			// collect lbls entry to be removed in the second phase
			info = GrB_Matrix_setElement_BOOL(tmp, true, id, j);
			ASSERT(info == GrB_SUCCESS);
	...
	info = GxB_rowIterator_attach(iter, tmp, NULL);
	ASSERT(info == GrB_SUCCESS);
	...
	while(info != GxB_EXHAUSTED) {
		...
		info = RG_Matrix_removeElement_BOOL(lbls, i, j);
		ASSERT(info == GrB_SUCCESS);
...
(
	...
	for (uint i = 0; i < count; i++) {
		...
		info = RG_Matrix_removeEntry_UINT64(R, src_id, dest_id,
				ENTITY_GET_ID(e), &entry_deleted);
		ASSERT(info == GrB_SUCCESS);
