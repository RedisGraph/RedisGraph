...
static inline int _MGraph_Bulk_Begin(RedisModuleCtx *ctx, RedisModuleString ***argv, int *argc,
									 RedisModuleString *rs_graph_name, const char *graphname) {
	...
	RedisModuleKey *key = RedisModule_OpenKey(ctx, rs_graph_name, REDISMODULE_READ);
...
static void _MGraph_BulkInsert(void *args) {
	...
	if(_MGraph_Bulk_Begin(ctx, &argv, &argc, rs_graph_name, graphname) != BULK_OK) goto cleanup;
	...
	if(rc == BULK_FAIL) {
		...
		RedisModule_ThreadSafeContextLock(ctx);
		RedisModuleKey *key = RedisModule_OpenKey(ctx, rs_graph_name, REDISMODULE_WRITE);
...
int MGraph_BulkInsert(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
	...
	// invoke bulk-insert on a bulk worker thread
	ThreadPools_AddWorkBulkLoader(_MGraph_BulkInsert, bulk_ctx);
