...
(
	...
	for(uint64_t i = 0; i < edge_count; i++) {
		...
		uint64_t  ids_count  =  RedisModule_LoadUnsigned(rdb);
		...
		if(ids_count == 1) {
			...
			EdgeID *edgeIds = array_new(EdgeID, ids_count);
			for (uint j = 0; j < ids_count; j++) {
				EdgeID edgeId = RedisModule_LoadUnsigned(rdb);
			...
			FormMultiConnection(gc->g, srcId, destId, edgeIds, relation);
		}
	}
}