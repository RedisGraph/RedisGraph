...
(
	...
	if(s) {
		...
		if(s->index) Index_Populate_enforce_constraint(s->index, NULL, (struct GraphContext*)gc);
		if(s->fulltextIdx) Index_Populate_enforce_constraint(s->fulltextIdx, NULL, (struct GraphContext*)gc);
	}
