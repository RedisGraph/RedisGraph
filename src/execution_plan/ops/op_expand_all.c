...
OpBase* NewExpandAllOp(RedisModuleCtx *ctx, QueryGraph *g, const char *graph_name,
                       Node **src_node, Edge **relation, Node **dest_node) {
    
    return (OpBase*)NewExpandAll(ctx, g, graph_name, src_node, relation, dest_node);
}

ExpandAll* NewExpandAll(RedisModuleCtx *ctx, QueryGraph *g, const char *graph_name,
                        Node **src_node, Edge **relation, Node **dest_node) {
    ...
    modified = QueryGraph_GetNodeAlias(g, *src_node);
    Vector_Push(expand_all->op.modifies, modified);
    modified = QueryGraph_GetEdgeAlias(g, *relation);
    Vector_Push(expand_all->op.modifies, modified);
    modified = QueryGraph_GetNodeAlias(g, *dest_node);
    Vector_Push(expand_all->op.modifies, modified);
...
OpResult ExpandAllConsume(OpBase *opBase, QueryGraph* graph) {
