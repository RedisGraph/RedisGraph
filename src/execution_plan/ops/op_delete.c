...
// OpBase* NewDeleteOp(RedisModuleCtx *ctx, AST_DeleteNode *ast_delete_node, QueryGraph *graph,
//                     const char *graph_name, ResultSet *result_set) {
//     return (OpBase*)_NewDeleteOp(ctx, ast_delete_node, graph, graph_name, result_set);
// }

// OpDelete* _NewDeleteOp(RedisModuleCtx *ctx, AST_DeleteNode *ast_delete_node, QueryGraph *graph,
//                        const char *graph_name, ResultSet *result_set) {
...
// void _LocateEntities(OpDelete *op, QueryGraph *g, AST_DeleteNode *ast_delete_node) {
//     for(int i = 0; i < Vector_Size(ast_delete_node->graphEntities); i++) {
//         char *entity_alias;
//         Vector_Get(ast_delete_node->graphEntities, i, &entity_alias);

//         Node *n = QueryGraph_GetNodeByAlias(g, entity_alias);
//         if (n != NULL) {
//             Node** node_ref = QueryGraph_GetNodeRef(g, n);
//             op->nodes_to_delete[op->node_count++] = node_ref;
//             continue;
//         }
//         Edge *e = QueryGraph_GetEdgeByAlias(g, entity_alias);
//         if(e != NULL) {
//             Edge **edge_ref = QueryGraph_GetEdgeRef(g, e);
