...
void NewExpandIntoOp(RedisModuleCtx *ctx, QueryGraph *g, const char *graph_name, Node **src_node,
                     Edge **relation, Node **dest_node, OpBase **op) {
    *op = (OpBase *)NewExpandInto(ctx, g, graph_name, src_node, relation, dest_node);
}

ExpandInto* NewExpandInto(RedisModuleCtx *ctx, QueryGraph *g, const char *graph_name, Node **src_node,
    ...
    Vector_Push(expandInto->op.modifies, QueryGraph_GetEdgeAlias(g, *relation));

    return expandInto;
}

OpResult ExpandIntoConsume(OpBase *opBase, QueryGraph* graph) {
