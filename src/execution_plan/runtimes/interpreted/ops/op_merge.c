...
// Merge logic
//------------------------------------------------------------------------------
static inline Record _pullFromStream(RT_OpBase *branch) {
...
RT_OpBase *RT_NewMergeOp(const RT_ExecutionPlan *plan, const OpMerge *op_desc) {
	...
	if(op_desc->on_match) {
		op->on_match = raxCloneWithCallback(op_desc->on_match,
			(void *(*)(void *))UpdateCtx_Clone);
		_InitializeUpdates(op, op->on_match, &op->on_match_it);
	}
	if(op_desc->on_create) {
		op->on_create = raxCloneWithCallback(op_desc->on_create,
			(void *(*)(void *))UpdateCtx_Clone);
...
static RT_OpResult MergeInit(RT_OpBase *opBase) {
	...
	if(opBase->childCount == 2) {
		...
		if(_LocateOp(opBase->children[0], OPType_MERGE_CREATE)) {
			...
			RT_OpBase *tmp = opBase->children[0];
	...
	op->match_argument_tap = (RT_Argument *)RT_ExecutionPlan_LocateOp(op->match_stream, OPType_ARGUMENT);
	// If the create stream is populated by an Argument tap, store a reference to it.
	op->create_argument_tap = (RT_Argument *)RT_ExecutionPlan_LocateOp(op->create_stream,
																 OPType_ARGUMENT);
...
static Record _handoff(RT_OpMerge *op) {
	Record r = NULL;
...
static Record MergeConsume(RT_OpBase *opBase) {
	...
	while(reading_matches) {
		...
		if(op->input_records) {
			...
			Argument_AddRecord(op->match_argument_tap, RT_OpBase_CloneRecord(lhs_record));
		} else {
		...
		if(lhs_record) RT_OpBase_DeleteRecord(lhs_record);
	}
	...
	if(op->bound_variable_stream) RT_OpBase_PropagateFree(op->bound_variable_stream);
	RT_OpBase_PropagateFree(op->match_stream);
...
static void MergeFree(RT_OpBase *opBase) {
	...
	if(op->input_records) {
		...
		for(uint i = 0; i < input_count; i ++) {
			RT_OpBase_DeleteRecord(op->input_records[i]);
		}
	...
	if(op->output_records) {
		...
		for(uint i = 0; i < output_count; i ++) {
			RT_OpBase_DeleteRecord(op->output_records[i]);
		}
