...
//				if(s) array_append(ctx->reltypes, s->id);
//			}
//
...
//			ctx->R = Graph_GetRelationMatrix(gc->g, ctx->reltypes[0], false);
//			if(maintain_transposes) ctx->TR = Graph_GetRelationMatrix(gc->g, ctx->reltypes[0], true);
...
//
//			for(uint i = 0; i < ctx->reltype_count; i ++) {
//				GrB_Matrix adj = Graph_GetRelationMatrix(gc->g, ctx->reltypes[i], false);
//				res = GrB_eWiseAdd(ctx->R, GrB_NULL, GrB_NULL, GxB_ANY_PAIR_BOOL, ctx->R, adj, GrB_NULL);
//				ASSERT(res == GrB_SUCCESS);
//				if(maintain_transposes) {
//					GrB_Matrix adj = Graph_GetRelationMatrix(gc->g, ctx->reltypes[i], true);
