...
int Record_AddToMapping(Record r, const char *alias) {
	/* Make sure alias has an entry associated with it
	 * within the record mapping. */
	// TODO if deleting local variable on every invocation, can use commented block.
	// Don't strictly need to delete variable for result set, which uses column map,
	// but could possible cause problems elsewhere? Record_Merge, etc
	/*
	assert(raxFind(r->mapping, (unsigned char *)alias, strlen(alias)) == raxNotFound);
	intptr_t id = raxSize(r->mapping);
	raxInsert(r->mapping, (unsigned char *)alias, strlen(alias), (void *)id, NULL);
	*/

	void *id = raxFind(r->mapping, (unsigned char *)alias, strlen(alias));
	if(id == raxNotFound) {
		id = (void *)raxSize(r->mapping);
		raxInsert(r->mapping, (unsigned char *)alias, strlen(alias), id, NULL);
	}

	return (intptr_t)id;
}

void Record_RemoveFromMapping(Record r, const char *alias) {
	raxRemove(r->mapping, (unsigned char *)alias, strlen(alias), NULL);
}
