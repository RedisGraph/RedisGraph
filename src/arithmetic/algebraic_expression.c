...
static void _referred_variable_length_edges(const QueryGraph *q,
...
static inline void _AlgebraicExpression_Execute_MUL(GrB_Matrix C, GrB_Matrix A,
        GrB_Matrix B, GrB_Descriptor desc) {
...
void AlgebraicExpression_AppendTerm(AlgebraicExpression *ae, GrB_Matrix m,
	...
	if(ae->operand_count + 1 > ae->operand_cap) {
		ae->operand_cap += 4;
		ae->operands = realloc(ae->operands,
		                       sizeof(AlgebraicExpressionOperand) * ae->operand_cap);
	}
...
void AlgebraicExpression_AppendOperand(AlgebraicExpression *ae,
                                       AlgebraicExpressionOperand op) {
	assert(ae);
...
void AlgebraicExpression_PrependOperand(AlgebraicExpression *ae,
                                        AlgebraicExpressionOperand op) {
	assert(ae);
...
static AlgebraicExpressionOperand _AlgebraicExpression_OperandFromNode(
    Node *n) {
	AlgebraicExpressionOperand op;
	op.free = false;
	op.diagonal = true;
...
static AlgebraicExpressionOperand _AlgebraicExpression_OperandFromEdge(
	...
	AST_LinkEntity *astEdge = (AST_LinkEntity *)MatchClause_GetEntity(
	                              ast->matchNode, e->alias);
	assert(astEdge);
	...
	if(labelCount > 1) {
		...
		GrB_Matrix m;
		GrB_Matrix_new(&m, GrB_BOOL, Graph_RequiredMatrixDim(g),
		...
		for(int i = 0; i < labelCount; i++) {
			...
			GrB_Info info = GrB_eWiseAdd_Matrix_Semiring(m, NULL, NULL, Rg_structured_bool,
			                m, l, NULL);
		}
		mat = m;
	}
...
static AlgebraicExpression *_AlgebraicExpression_From_Path(Edge **path,
        uint path_len, const AST *ast) {
...
AlgebraicExpression **AlgebraicExpression_From_QueryGraph(const QueryGraph *qg,
        const AST *ast, size_t *exp_count) {
	assert(qg && ast);
	...
	while(g->node_count) {
		...
		AlgebraicExpression **sub_exps = _AlgebraicExpression_Intermidate_Expressions(
		                                     exp, path, g, ref_entities);
	...
	*exp_count = array_len(exps);
	AlgebraicExpression **res = rm_malloc(sizeof(AlgebraicExpression *) *
	                                      (*exp_count));
...
void AlgebraicExpression_Execute(AlgebraicExpression *ae, GrB_Matrix res) {
	...
	memcpy(operands, ae->operands,
	       sizeof(AlgebraicExpressionOperand) * operand_count);
	...
	for(int i = 1; i < operand_count; i++) {
		...
		_AlgebraicExpression_Execute_MUL(res, leftTerm.operand, rightTerm.operand,
		                                 GrB_NULL);
	...
	if((*root)->type == AL_OPERATION && (*root)->operation.op == AL_EXP_MUL) {
		...
		if((l->type == AL_OPERATION && l->operation.op == AL_EXP_ADD &&
			...
			AlgebraicExpressionNode *add = AlgebraicExpressionNode_NewOperationNode(
			                                   AL_EXP_ADD);
			AlgebraicExpressionNode *lMul = AlgebraicExpressionNode_NewOperationNode(
			                                    AL_EXP_MUL);
			AlgebraicExpressionNode *rMul = AlgebraicExpressionNode_NewOperationNode(
			                                    AL_EXP_MUL);
...
// Forward declaration.
static GrB_Matrix _AlgebraicExpression_Eval(AlgebraicExpressionNode *exp,
        GrB_Matrix res);

static GrB_Matrix _AlgebraicExpression_Eval_ADD(AlgebraicExpressionNode *exp,
        GrB_Matrix res) {
...
static GrB_Matrix _AlgebraicExpression_Eval_MUL(AlgebraicExpressionNode *exp,
        GrB_Matrix res) {
	// Expression already evaluated.
	if(exp->operation.v != NULL) return exp->operation.v;
...
static GrB_Matrix _AlgebraicExpression_Eval_TRANSPOSE(AlgebraicExpressionNode
        *exp, GrB_Matrix res) {
