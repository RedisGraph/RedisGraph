...
void Register_StringFuncs() {
	...
	func_desc = AR_FuncDescNew("left", AR_LEFT, 2, types);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, (T_STRING | T_NULL));
	func_desc = AR_FuncDescNew("ltrim", AR_LTRIM, 1, types);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 2);
	types = array_append(types, (T_STRING | T_NULL));
	types = array_append(types, T_INT64);
	func_desc = AR_FuncDescNew("right", AR_RIGHT, 2, types);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, (T_STRING | T_NULL));
	func_desc = AR_FuncDescNew("rtrim", AR_RTRIM, 1, types);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, (T_STRING | T_NULL));
	func_desc = AR_FuncDescNew("reverse", AR_REVERSE, 1, types);
	AR_RegFunc(func_desc);
	...
	types = array_append(types, T_INT64);
	types = array_append(types, T_INT64);
	func_desc = AR_FuncDescNew("substring", AR_SUBSTRING, VAR_ARG_LEN, types);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, (T_STRING | T_NULL));
	func_desc = AR_FuncDescNew("tolower", AR_TOLOWER, 1, types);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, (T_STRING | T_NULL));
	func_desc = AR_FuncDescNew("toupper", AR_TOUPPER, 1, types);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, SI_ALL);
	func_desc = AR_FuncDescNew("tostring", AR_TOSTRING, 1, types);
	AR_RegFunc(func_desc);
	...
	types = array_append(types, (T_STRING | T_NULL));
	types = array_append(types, (T_STRING | T_NULL));
	func_desc = AR_FuncDescNew("contains", AR_CONTAINS, 2, types);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 2);
	types = array_append(types, (T_STRING | T_NULL));
	types = array_append(types, (T_STRING | T_NULL));
	func_desc = AR_FuncDescNew("starts with", AR_STARTSWITH, 2, types);
	AR_RegFunc(func_desc);
	...
	func_desc = AR_FuncDescNew("randomuuid", AR_RANDOMUUID, 0, types);
	AR_RegFunc(func_desc);
}
