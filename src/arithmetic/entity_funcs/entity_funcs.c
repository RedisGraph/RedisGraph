...
void Register_EntityFuncs() {
	...
	func_desc = AR_FuncDescNew(AR_ID, 1, types);
	AR_RegFunc("id", func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, T_NODE);
	func_desc = AR_FuncDescNew(AR_LABELS, 1, types);
	AR_RegFunc("labels", func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, T_EDGE);
	func_desc = AR_FuncDescNew(AR_TYPE, 1, types);
	AR_RegFunc("type", func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, T_NULL | T_BOOL | SI_NUMERIC | SI_STRING | T_NODE | T_EDGE);
	func_desc = AR_FuncDescNew(AR_EXISTS, 1, types);
	AR_RegFunc("exists", func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, T_NODE);
	func_desc = AR_FuncDescNew(AR_INCOMEDEGREE, 1, types);
	AR_RegFunc("indegree", func_desc);

	types = array_new(SIType, 1);
	types = array_append(types, T_NODE);
	func_desc = AR_FuncDescNew(AR_OUTGOINGDEGREE, 1, types);
	AR_RegFunc("outdegree", func_desc);
}
