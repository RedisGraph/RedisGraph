...
void Register_EntityFuncs() {
	...
	func_desc = AR_FuncDescNew("id", AR_ID, 1, 1, types, ret_type, true);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, T_NULL | T_NODE);
	ret_type = T_NULL | T_ARRAY;
	func_desc = AR_FuncDescNew("labels", AR_LABELS, 1, 1, types, ret_type, true);
	AR_RegFunc(func_desc);
	...
	func_desc = AR_FuncDescNew("hasLabels", AR_HAS_LABELS, 2, 2, types, ret_type, false);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, T_NULL | T_EDGE);
	ret_type = T_NULL | T_STRING;
	func_desc = AR_FuncDescNew("type", AR_TYPE, 1, 1, types, ret_type, true);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, T_NULL | T_EDGE);
	ret_type = T_NULL | T_NODE;
	func_desc = AR_FuncDescNew("startNode", AR_STARTNODE, 1, 1, types, ret_type, true);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, T_NULL | T_EDGE);
	ret_type = T_NULL | T_NODE;
	func_desc = AR_FuncDescNew("endNode", AR_ENDNODE, 1, 1, types, ret_type, true);
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, T_NULL | SI_ALL);
	ret_type = T_NULL | T_BOOL;
	func_desc = AR_FuncDescNew("exists", AR_EXISTS, 1, 1, types, ret_type, true);
	AR_RegFunc(func_desc);
	...
	ret_type = T_NULL | T_INT64;
	func_desc = AR_FuncDescNew("indegree", AR_INCOMEDEGREE, 1, VAR_ARG_LEN, types, ret_type, true);
	AR_RegFunc(func_desc);
	...
	array_append(types, T_STRING);
	ret_type = T_NULL | T_INT64;
	func_desc = AR_FuncDescNew("outdegree", AR_OUTGOINGDEGREE, 1, VAR_ARG_LEN, types, ret_type, true);
	AR_RegFunc(func_desc);
