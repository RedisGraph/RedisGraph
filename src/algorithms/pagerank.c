...
(
	...
	float x = 1.0 / ((float) n) ;
	info = GrB_Vector_new(&r, GrB_FP32, n) ;
	assert(info == GrB_SUCCESS) ;

	info = GrB_assign(r, NULL, NULL, x, GrB_ALL, n, NULL) ;
	assert(info == GrB_SUCCESS) ;

	// d (i) = out deg of node i
	info = GrB_Vector_new(&d, GrB_FP32, n) ;
	assert (info == GrB_SUCCESS) ;

	info = GrB_reduce(d, NULL, NULL, GrB_PLUS_FP32, A, NULL) ;
	assert(info == GrB_SUCCESS) ;
	// GxB_print (d, 3) ;
	...
	info = GxB_Vector_export_CSC(&d, &type, &n, &I, (void **)(&X), &vi_size,
	...
	info = GrB_Matrix_new(&C, GrB_FP32, n, n) ;
	assert(info == GrB_SUCCESS) ;
	// GxB_set (C, GxB_HYPER, GxB_ALWAYS_HYPER) ;

	for(int64_t k = 0 ; k < n ; k++) {
		// C(k,k) = 0
		info = GrB_Matrix_setElement(C, (float) 0, k, k) ;
	   	assert(info == GrB_SUCCESS) ;
	}

	// make sure D is diagonal
	info = GrB_eWiseAdd(D, NULL, NULL, GrB_PLUS_FP32, D, C, NULL) ;
	assert(info == GrB_SUCCESS) ;
	...
	// T = D*A
	info = GrB_Matrix_new(&T, GrB_FP32, n, n) ;
	assert(info == GrB_SUCCESS);

	info = GrB_mxm(T, NULL, NULL, GxB_PLUS_TIMES_FP32, D, A, NULL) ;
	assert(info == GrB_SUCCESS) ;

	// C = C+T'
	info = GrB_transpose(C, NULL, GrB_PLUS_FP32, T, NULL) ;
	assert(info == GrB_SUCCESS) ;
	...
	for((*iters) = 0 ; (*iters) < itermax && rdiff > ftol ; (*iters)++) {
		...
		info = GrB_reduce(&rsum, NULL, GxB_PLUS_FP32_MONOID, r, NULL) ;
		assert(info == GrB_SUCCESS) ;

		// t = C*r
		// using the transpose of A, scaled (dot product)
		info = GrB_mxv(t, NULL, NULL, GxB_PLUS_TIMES_FP32, C, r, NULL) ;
		assert(info == GrB_SUCCESS) ;

		// t += teleport_scalar ;
		float teleport_scalar = teleport * rsum ;
		info = GrB_assign(t, NULL, GrB_PLUS_FP32, teleport_scalar, GrB_ALL, n, NULL) ;
		assert(info == GrB_SUCCESS) ;
		//----------------------------------------------------------------------
		// rdiff = sum ((r-t).^2)
		//----------------------------------------------------------------------

		info = GrB_eWiseAdd(r, NULL, NULL, op_diff, r, t, NULL) ;
		assert(info == GrB_SUCCESS) ;
		info = GrB_reduce(&rdiff, NULL, GxB_PLUS_FP32_MONOID, r, NULL) ;
		assert(info == GrB_SUCCESS) ;
	...
	// r = r / rsum
	info = GrB_Vector_assign_FP32(r, NULL, GrB_TIMES_FP32, 1 / rsum, GrB_ALL, n, NULL) ;
	assert(info == GrB_SUCCESS) ;
	...
	info = GxB_Vector_export_CSC(&r, &type, &n, &I, (void **)(&X), &vi_size,
				&vx_size, &iso, &nvals, &jumbled, NULL) ;
	assert(info == GrB_SUCCESS) ;
