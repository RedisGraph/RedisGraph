...
static void _AST_MapReferencedEntitiesInPath(AST *ast, const cypher_astnode_t *path, bool override_force_mapping);
...
static void _AST_MapExpression(AST *ast, const cypher_astnode_t *exp) {
	...
	if(type == CYPHER_AST_IDENTIFIER) {
		...
		const cypher_astnode_t *pattern = cypher_ast_pattern_comprehension_get_pattern(exp);
		// Reference all entity names in pattern comprehension.
...
static void _AST_MapReferencedEntitiesInPath(AST *ast, const cypher_astnode_t *path, bool override_force_mapping) {
	...
	bool force_mapping = override_force_mapping || (type == CYPHER_AST_NAMED_PATH || type == CYPHER_AST_SHORTEST_PATH);
...
static void _AST_MapMatchClauseReferences(AST *ast, const cypher_astnode_t *match_clause) {
	...
	for(uint i = 0; i < path_count; i++) {
		...
		_AST_MapReferencedEntitiesInPath(ast, path, false);
	}
...
static void _AST_MapMergeClauseReference(AST *ast, const cypher_astnode_t *merge_clause) {
	...
	_AST_MapReferencedEntitiesInPath(ast, merge_path, false);
