...
| algo.BFS                        | `source alias`, `max level`, `relationship-type`, `yield-edges` | `start_node`, `nodes`, `edges` | Performs BFS to find all nodes connected to source and their relative level. A `max level` of 0 indicates unlimited and a non-NULL `relationship-type` defines the relationship type that may be traversed. |
