...
In RedisGraph, each match to a variable-length path is a sequence of nodes and edges that starts and ends with a relationship, where the head and tail nodes may not appear, and no node may appears more than once . Note that other OpenCypher implementations may use different definitions (e.g., no edge may appear more than once).
