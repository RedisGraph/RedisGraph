...
| string.distance(str1, str2, distFunc = 'Lev', distFuncParams = {}) → floating-point                         | Given two strings, return their distance according to a specified distance function. <br> distFunc value is case insensitive. <br> distFuncParams is a map specifying the distance function parameters as detailed below. Unexpected map keys are ignored. <br> If distFunc is 'Lev': <br> Return the Levenshtein distance <br> distFuncParams, if specified, should be a map. It may contain InsertionWeight, DeletionWeight, and SubstitutionWeight - all nonnegative floating-point values (default values: 1,1,1). Note that if InsertionWeight and SubstitutionWeight are not equal, the distance is non-commutative and equals to the edit distance from str1 to str2. <br> If distFunc is 'DamLev': Return the Damerau-Levenshtein distance <br> distFuncParams, if specified, should be a map. It may contain InsertionWeight, DeletionWeight, SubstitutionWeight, and transpositionWeight - all nonnegative floating-point values (default values: 1,1,1,1). Note that if InsertionWeight and SubstitutionWeight are not equal, the distance is non-commutative and equals to the edit distance from str1 to str2. <br> If distFunc is 'Ham': <br> Return the Hamming distance, or -1 if str and str2 have different lengths. <br> distFuncParams, if specified, should be a map, but is not used. <br> If distFunc is 'Jaro': <br> Return the Jaro distance <br> distFuncParams, if specified, should be a map, but is not used. <br> If distFunc is 'JaroW': <br> Return the Jaro-Winkler distance
<br> distFuncParams, if specified, should be a map. It may contain sacleFactor - a floating-point value [0 .. 0.25] (default value: 0.1) <br> Returns null when str1 evaluates to null <br> Returns null when str2 evaluates to null                                                                        |
&#42; RedisGraph-specific extensions to Cypher
