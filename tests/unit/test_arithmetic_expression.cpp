...
TEST_F(ArithmeticTest, LogTest) {
	...
	query = "RETURN LOG(10)";
	arExp = _exp_from_query(query);
	expected = SI_DoubleVal(log(10));
	_test_ar_func(arExp, expected);
	AR_EXP_Free(arExp);

	/* LOG10(1) */
	query = "RETURN LOG10(1)";
	arExp = _exp_from_query(query);
	expected = SI_DoubleVal(0);
	_test_ar_func(arExp, expected);
	AR_EXP_Free(arExp);
