...
// matrices
QueryGraph  *qg         =  NULL;
RG_Matrix   mat_p       =  NULL;
RG_Matrix   mat_ef      =  NULL;
RG_Matrix   mat_tef     =  NULL;
RG_Matrix   mat_f       =  NULL;
RG_Matrix   mat_ev      =  NULL;
RG_Matrix   mat_tev     =  NULL;
RG_Matrix   mat_c       =  NULL;
RG_Matrix   mat_ew      =  NULL;
RG_Matrix   mat_tew     =  NULL;
RG_Matrix   mat_e       =  NULL;
rax         *_matrices  =  NULL;
...
void _compare_algebraic_operand(AlgebraicExpression *a, AlgebraicExpression *b) {
	TEST_CHECK(a->type == AL_OPERAND);
	TEST_CHECK(b->type == AL_OPERAND);
	TEST_CHECK(a->operand.matrix == b->operand.matrix);
}

void compare_algebraic_expression(AlgebraicExpression *a, AlgebraicExpression *b) {
	TEST_CHECK(a->type == b->type);
	TEST_CHECK(AlgebraicExpression_ChildCount(a) == AlgebraicExpression_ChildCount(b));
	TEST_CHECK(AlgebraicExpression_OperandCount(a) == AlgebraicExpression_OperandCount(b));
...
void test_algebraicExpression_new(void) {
	setup();
	...
	AlgebraicExpression *operand = AlgebraicExpression_NewOperand(matrix,
			diagonal, src, dest, edge, label);

	TEST_CHECK(operand->type == AL_OPERAND);
	TEST_CHECK(operand->operand.matrix == matrix);
	TEST_CHECK(operand->operand.diagonal == diagonal);
	TEST_CHECK(operand->operand.src == src);
	TEST_CHECK(operand->operand.dest == dest);
	TEST_CHECK(operand->operand.edge == edge);
	TEST_CHECK(operand->operand.label == label);

	TEST_CHECK(AlgebraicExpression_Source(operand) == src);
	TEST_CHECK(AlgebraicExpression_Destination(operand) == dest);
	TEST_CHECK(AlgebraicExpression_Edge(operand) == edge);
	TEST_CHECK(AlgebraicExpression_ChildCount(operand) == 0);
	TEST_CHECK(AlgebraicExpression_OperandCount(operand) == 1);
	TEST_CHECK(AlgebraicExpression_Transposed(operand) == false);
	...
	TEST_CHECK(operation->type == AL_OPERATION);
	TEST_CHECK(operation->operation.op == op);
	TEST_CHECK(array_len(operation->operation.children) == 1);

	TEST_CHECK(AlgebraicExpression_Source(operation) == src);
	TEST_CHECK(AlgebraicExpression_Destination(operation) == dest);
	TEST_CHECK(AlgebraicExpression_Edge(operation) == edge);
	TEST_CHECK(AlgebraicExpression_ChildCount(operation) == 1);
	TEST_CHECK(AlgebraicExpression_OperandCount(operation) == 1);
	TEST_CHECK(AlgebraicExpression_Transposed(operation) == false);
...
void test_algebraicExpression_domains(void) {
	setup();
	...
	const  char  *src_domain      =  NULL;  // row domain of expression
	const  char  *dest_domain     =  NULL;  // column domain of expression
	const  char  *expressions[15]   =  { "A", "A+B", "B+A", "A*B", "B*A", "T(A)", "T(A+B)", "T(B+A)", "T(A*B)", "T(B*A)", "T(T(A))", "T(T(A+B))", "T(T(B+A))", "T(T(A*B))", "T(T(B*A))" };
	const  char  *sources[15]       =  { "A", "A",   "B",   "A",   "B",   "A",    "A",      "B",      "B",      "A",      "A",       "A",          "B",        "A",         "B" };
	const  char  *destinations[15]  =  { "A", "A",   "B",   "B",   "A",   "A",    "A",      "B",      "A",      "B",      "A",       "A",          "B",        "B",         "A" };
	...
	for(int i = 0; i < 15; i++) {
		...
		const  char  *dest        =  destinations[i];
	
		exp          =  AlgebraicExpression_FromString(expression, matrices);
		src_domain   =  AlgebraicExpression_Source(exp);
		dest_domain  =  AlgebraicExpression_Destination(exp);

		TEST_CHECK(strcmp(src, src_domain) == 0);
		TEST_CHECK(strcmp(dest, dest_domain) == 0);
...
void test_algebraicExpression_clone(void) {
	setup();
...
void test_algebraicExpression_transpose(void) {
	setup();
...
void test_algebraicExpression_Exp_OP_ADD(void) {
	setup();
	...
	// A + B = C.
	TEST_CHECK(_compare_matrices(expected, res) == true);
...
void test_algebraicExpression_Exp_OP_MUL(void) {
	setup();
...
void test_algebraicExpression_Exp_OP_ADD_Transpose(void) {
	setup();
...
void test_algebraicExpression_Exp_OP_MUL_Transpose(void) {
	setup();
	...
	// Using the A matrix described above,
	// Transpose(A) * A = B.
	TEST_CHECK(_compare_matrices(B, res) == true);
...
void test_algebraicExpression_Exp_OP_A_MUL_B_Plus_C(void) {
	setup();
...
void test_algebraicExpression_ExpTransform_A_Times_B_Plus_C(void) {
	setup();
...
void test_algebraicExpression_ExpTransform_AB_Times_C_Plus_D(void) {
	setup();
	...
	TEST_CHECK(leftchild_0->type == AL_OPERAND && leftchild_0->operand.matrix == A);

	AlgebraicExpression *leftchild_1 = rootLeftChild->operation.children[1];
	TEST_CHECK(leftchild_1->type == AL_OPERAND && leftchild_1->operand.matrix == B);

	AlgebraicExpression *leftchild_2 = rootLeftChild->operation.children[2];
	TEST_CHECK(leftchild_2->type == AL_OPERAND && leftchild_2->operand.matrix == C);

	AlgebraicExpression *rightchild_0 = rootRightChild->operation.children[0];
	TEST_CHECK(rightchild_0->type == AL_OPERAND && rightchild_0->operand.matrix == A);

	AlgebraicExpression *rightchild_1 = rootRightChild->operation.children[1];
	TEST_CHECK(rightchild_1->type == AL_OPERAND && rightchild_1->operand.matrix == B);

	AlgebraicExpression *rightchild_2 = rootRightChild->operation.children[2];
	TEST_CHECK(rightchild_2->type == AL_OPERAND && rightchild_2->operand.matrix == D);
...
void test_algebraicExpression_ExpTransform_A_Plus_B_Times_C_Plus_D(void) {
	setup();
	...
	TEST_CHECK(strcmp("(((A * C + A * D) + B * C) + B * D)", exp_str) == 0);
	rm_free(exp_str);
	raxFree(matrices);
	RG_Matrix_free(&A);
	RG_Matrix_free(&B);
...
void test_algebraicExpression_MultipleIntermidateReturnNodes(void) {
	setup();
...
void test_algebraicExpression_OneIntermidateReturnNode(void) {
	setup();

	const char *q = query_one_intermidate_return_nodes;
	AlgebraicExpression **actual = build_algebraic_expression(q);
	uint exp_count = array_len(actual);
	TEST_CHECK(exp_count == 2);
...
void test_algebraicExpression_NoIntermidateReturnNodes(void) {
	setup();

	const char *q = query_no_intermidate_return_nodes;
	AlgebraicExpression **actual = build_algebraic_expression(q);
	uint exp_count = array_len(actual);
	TEST_CHECK(exp_count == 1);
...
void test_algebraicExpression_OneIntermidateReturnEdge(void) {
	setup();
...
void test_algebraicExpression_BothDirections(void) {
	setup();
...
void test_algebraicExpression_SingleNode(void) {
	setup();

	const char *q = "MATCH (p:Person) RETURN p";
	AlgebraicExpression **actual = build_algebraic_expression(q);
	uint exp_count = array_len(actual);
	TEST_CHECK(exp_count == 0);
...
void test_algebraicExpression_ShareableEntity(void) {
	setup();
...
void test_algebraicExpression_VariableLength(void) {
	setup();
...
void test_algebraicExpression_ExpressionExecute(void) {
	setup();
	...
	TEST_CHECK(strcmp(AlgebraicExpression_Source(exp), "p") == 0);
	TEST_CHECK(strcmp(AlgebraicExpression_Destination(exp), "e") == 0);
	...
	TEST_CHECK(_compare_matrices(expected, res));
...
void test_algebraicExpression_RemoveOperand(void) {
	setup();
	...
	for(int i = 0; i < 13; i++) {
		...
		if(expected == NULL) TEST_CHECK(NULL == exp);
...
void test_algebraicExpression_LocateOperand(void) {
	setup();
	...
	TEST_CHECK(located);
	TEST_CHECK(op->type == AL_OPERAND);
	TEST_CHECK(strcmp(op->operand.src, "a") == 0);
	TEST_CHECK(strcmp(op->operand.dest, "b") == 0);
	TEST_CHECK(strcmp(op->operand.edge, "e0") == 0);

	TEST_CHECK(p->type == AL_OPERATION);
	TEST_CHECK(p->operation.op == AL_EXP_TRANSPOSE);

	// search for none existing operand
	TEST_CHECK(!AlgebraicExpression_LocateOperand(r, &op, &p, "x", "b", "e0"));
	TEST_CHECK(!AlgebraicExpression_LocateOperand(r, &op, &p, "a", "x", "e0"));
	TEST_CHECK(!AlgebraicExpression_LocateOperand(r, &op, &p, "a", "b", "x"));
	TEST_CHECK(!AlgebraicExpression_LocateOperand(r, &op, &p, NULL, "b", "e0"));
	TEST_CHECK(!AlgebraicExpression_LocateOperand(r, &op, &p, "a", NULL, "e0"));
	TEST_CHECK(!AlgebraicExpression_LocateOperand(r, &op, &p, "a", "b", NULL));
...
TEST_LIST = {
	{  "algebraicExpression_new",                                   test_algebraicExpression_new                                   },
	...
	{  "algebraicExpression_Exp_OP_ADD",                            test_algebraicExpression_Exp_OP_ADD                            },
	{  "algebraicExpression_Exp_OP_MUL",                            test_algebraicExpression_Exp_OP_MUL                            },
	{  "algebraicExpression_Exp_OP_ADD_Transpose",                  test_algebraicExpression_Exp_OP_ADD_Transpose                  },
	{  "algebraicExpression_Exp_OP_MUL_Transpose",                  test_algebraicExpression_Exp_OP_MUL_Transpose                  },
	{  "algebraicExpression_Exp_OP_A_MUL_B_Plus_C",                 test_algebraicExpression_Exp_OP_A_MUL_B_Plus_C                 },
	{  "algebraicExpression_ExpTransform_A_Times_B_Plus_C",         test_algebraicExpression_ExpTransform_A_Times_B_Plus_C         },
	{  "algebraicExpression_ExpTransform_AB_Times_C_Plus_D",        test_algebraicExpression_ExpTransform_AB_Times_C_Plus_D        },
	{  "algebraicExpression_ExpTransform_A_Plus_B_Times_C_Plus_D",  test_algebraicExpression_ExpTransform_A_Plus_B_Times_C_Plus_D  },
	{  "algebraicExpression_MultipleIntermidateReturnNodes",        test_algebraicExpression_MultipleIntermidateReturnNodes        },
	{  "algebraicExpression_OneIntermidateReturnNode",              test_algebraicExpression_OneIntermidateReturnNode              },
	{  "algebraicExpression_NoIntermidateReturnNodes",              test_algebraicExpression_NoIntermidateReturnNodes              },
	{  "algebraicExpression_OneIntermidateReturnEdge",              test_algebraicExpression_OneIntermidateReturnEdge              },
	{  "algebraicExpression_BothDirections",                        test_algebraicExpression_BothDirections                        },
	{  "algebraicExpression_SingleNode",                            test_algebraicExpression_SingleNode                            },
	{  "algebraicExpression_ShareableEntity",                       test_algebraicExpression_ShareableEntity                       },
	{  "algebraicExpression_VariableLength",                        test_algebraicExpression_VariableLength                        },
	{  "algebraicExpression_ExpressionExecute",                     test_algebraicExpression_ExpressionExecute                     },
	{  "algebraicExpression_RemoveOperand",                         test_algebraicExpression_RemoveOperand                         },
	{  "algebraicExpression_LocateOperand",                         test_algebraicExpression_LocateOperand                         },
