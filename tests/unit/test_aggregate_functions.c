...
void test_count(void) {
	...
	TEST_CHECK(result.longval == num_values);

	AR_EXP_Free(arExp);
}

// count a mix of valid and invalid entities
void test_partial_count(void) {
	setup();
	...
	TEST_CHECK(res.longval == num_values / 2);
	AR_EXP_Free(arExp);
}

void test_percentile_cont(void) {
	setup();
	...
	for(int i = 0; i < 5; i++) {
		...
		TEST_CHECK(result.doubleval == expected_values[i]);
		AR_EXP_Free(perc);
	}
}

void test_percentile_disc(void) {
	setup();
	...
	for(int i = 0; i < 5; i++) {
		...
		TEST_CHECK(res.doubleval == expected_outcome.doubleval);
		AR_EXP_Free(perc);
	}
}

// tests both stDev and stDevP
void test_stdev(void) {
	setup();
	...
	TEST_CHECK(result.doubleval == sample_result);
	AR_EXP_Free(stdev);
	...
	TEST_CHECK(result.doubleval == pop_result);
	AR_EXP_Free(stdevp);
}

TEST_LIST = {
	{ "count", test_count },
	{ "partial_count", test_partial_count },
	{ "test_percentile_cont", test_percentile_cont },
