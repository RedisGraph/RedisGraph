...
class testIndexCreationFlow(FlowTestsBase):
    def __init__(self):
        ...
        redis_graph2 = Graph(social_utils.graph_name, redis_con)
        social_utils.populate_graph(redis_con, redis_graph2)
    ...
    def test02_multi_prop_index_creation(self):
        ...
        result = redis_graph2.query("CREATE INDEX ON :person(age, name)")
        self.env.assertEquals(result.indices_created, 2)
        ...
        result = redis_graph2.query("CREATE INDEX ON :person(age, age, name, height)")
        self.env.assertEquals(result.indices_created, 1)
        ...
        result = redis_graph2.query("CREATE INDEX ON :person(gender, gender, name, height)")
        self.env.assertEquals(result.indices_created, 1)