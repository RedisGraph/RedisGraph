...
class testEntityUpdate(FlowTestsBase):
    ...
    def test04_update_remove_attribute(self):
        ...
        result = graph.query("UNWIND [{id: 1, aField: 'withValue', andOneWithout: null}] AS item MERGE (m:X{id: item.id}) ON CREATE SET m += item RETURN m")
        ...
        properties = result.result_set[0][0].properties
        self.env.assertEqual(properties, expected_result)
