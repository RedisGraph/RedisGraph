...
class GraphTraversalsFlowTest(FlowTestsBase):
    ...
    def test02_edge_properties(self):
    ...
    def test04_single_labeled_hop(self):
    ...
    def test05_single_labeled_hop_loop_detection(self):
    ...
    def test06_single_hop_loop_detection(self):
    ...
    def test07_single_labeled_hop_loop_detection(self):
        query = """MATCH (a)-[*]->(a) return a ORDER BY a.val"""
        actual_result = redis_graph.query(query)
        print actual_result.result_set
        assert (actual_result.result_set[1:]) == [['a'], ['b']]

    def test08_multiple_sources(self):
        # Add a new node with an edge connecting to b
        query = """MATCH (b:nodeval {val: 'b'}) CREATE (d:nodeval {val: 'd'})-[:connects]->(b)"""
        actual_result = redis_graph.query(query)
        assert(actual_result.nodes_created == 1)
        assert(actual_result.relationships_created == 1)
        # Find entities with 2 unique sources pointing to the same destination
        query = """MATCH (a)-[]->(b)<-[]-(c) WHERE a.val != c.val RETURN a, b, c ORDER BY b.val, a.val, c.val"""
        actual_result = redis_graph.query(query)
        print actual_result.result_set
