...
#define GB_TEST_VALUE_OF_ENTRY(keep,p)                  \
    GB_void z [GB_VLA(zsize)] ; GB_void x [GB_VLA(xsize)] ; GB_cast_scalar (x, xcode, Ax +(p)*asize, acode, asize) ; op->idxunop_function (z, x, flipij ? j : i, flipij ? i : j, ythunk) ; bool keep ; GB_cast_scalar (&keep, GB_BOOL_code, z, zcode, zsize) ;
