...
)
{
    ...
    ztype *restrict w = GB_thread_local.Work ;
    ...
    for (int64_t j = 0 ; j < n ; j++)
    {
        ...
        for (int64_t kk = kk1 ; kk < kk2 ; kk++)
        {
            ...
            if (bjnz == nrows && ainz == nrows)
            {
                ...
                {
                    xytype aki = Ax [pa + k] ;      // aki = A(k,i)
                    xytype bkj = Bx [pb + k] ;      // bkj = B(k,j)
                    ztype t = MULT(aki, bkj) ;
                    ADD(cij, t) ;
                }
