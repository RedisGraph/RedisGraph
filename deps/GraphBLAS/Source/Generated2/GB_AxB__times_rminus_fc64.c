...
// A'*B (dot2):        GB (_Adot2B__times_rminus_fc64)
// A'*B (dot3):        GB (_Adot3B__times_rminus_fc64)
// C+=A'*B (dot4):     GB (_Adot4B__times_rminus_fc64)
// A*B (saxpy bitmap): GB (_AsaxbitB__times_rminus_fc64)
// A*B (saxpy3):       GB (_Asaxpy3B__times_rminus_fc64)
//     no mask:        GB (_Asaxpy3B_noM__times_rminus_fc64)
//     mask M:         GB (_Asaxpy3B_M__times_rminus_fc64)
//     mask !M:        GB (_Asaxpy3B_notM__times_rminus_fc64)

// C type:   GxB_FC64_t
// A type:   GxB_FC64_t
// B type:   GxB_FC64_t

// Multiply: z = GB_FC64_minus (bkj, aik)
// Add:      cij = GB_FC64_mul (cij, z)
//           'any' monoid?  0
//           atomic?        0
//           OpenMP atomic? 0
// MultAdd:  GxB_FC64_t x_op_y = GB_FC64_minus (bkj, aik) ; cij = GB_FC64_mul (cij, x_op_y)
...
#define GB_GETA(aik,Ax,pA,A_iso) \
    GxB_FC64_t aik = GBX (Ax, pA, A_iso)

// bkj = Bx [pB]
#define GB_GETB(bkj,Bx,pB,B_iso) \
    GxB_FC64_t bkj = GBX (Bx, pB, B_iso)
...
#define GB_CIJ_MEMCPY(p,i,len) \
    memcpy (Cx +(p), Hx +(i), (len) * sizeof(GxB_FC64_t));
...
//------------------------------------------------------------------------------
    ...
    { 
        ...
        return (GrB_NO_VALUE) ;
        #else
        const GxB_FC64_t cinput = (*((const GxB_FC64_t *) cinput_void)) ;
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__times_rminus_fc64) (C,
