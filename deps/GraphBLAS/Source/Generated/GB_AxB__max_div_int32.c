//------------------------------------------------------------------------------
// GB_AxB__max_div_int32:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__max_div_int32
// A'*B function: GB_AdotB__max_div_int32
// Z type :  int32_t (the type of C)
// XY type:  int32_t (the type of A and B)
// Identity: INT32_MIN (where cij = IMAX (cij,INT32_MIN) does not change cij)
...
)
{
    ...
    int32_t *restrict w = GB_thread_local.Work ;
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    int32_t aik = Ax [pa] ;
                    int32_t t = flip ? IDIV(bkj,aik) : IDIV(aik,bkj) ;
                    if (flag > 0)
                    {
...
)
{
    ...
    #define MERGE                                           \
    {                                                       \
        int32_t aki = Ax [pa++] ;    /* aki = A(k,i) */      \
        int32_t bkj = Bx [pb++] ;    /* bjk = B(k,j) */      \
        int32_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMAX (cij,t) ;                                   \
    ...
    int32_t *Cx = C->x ;
    const int32_t *Ax = A->x ;
    const int32_t *Bx = B->x ;
    ...
    for (int64_t j = 0 ; j < n ; j++)
    {
        ...
        for (int64_t kk = kk1 ; kk < kk2 ; kk++)
        {
            ...
            if (bjnz == nrows && ainz == nrows)
            {
                ...
                {
                    int32_t aki = Ax [pa + k] ;      // aki = A(k,i)
                    int32_t bkj = Bx [pb + k] ;      // bkj = B(k,j)
                    int32_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;
                    cij = IMAX (cij,t) ;
                }
