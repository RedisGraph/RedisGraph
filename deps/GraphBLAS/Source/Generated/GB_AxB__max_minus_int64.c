//------------------------------------------------------------------------------
// GB_AxB__max_minus_int64:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__max_minus_int64
// A'*B function: GB_AdotB__max_minus_int64
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    int64_t aik = Ax [pa] ;
                    int64_t t = flip ? (bkj-aik) : (aik-bkj) ;
                    if (flag > 0)
                    {
...
void GB_AdotB__max_minus_int64
(
    GrB_Matrix C,
    const GrB_Matrix Mask,
    const GrB_Matrix A,
...
)
{
    ...
    {                                                       \
        ...
        int64_t t = flip ? (bkj-aki) : (aki-bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMAX (cij,t) ;                                   \
