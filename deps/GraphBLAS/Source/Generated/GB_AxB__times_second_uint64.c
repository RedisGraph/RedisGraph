...
// A'*B function (dot):      GB_AdotB__times_second_uint64
// A*B function (heap):      GB_AheapB__times_second_uint64
// Z type:   uint64_t (the type of C)
// X type:   uint64_t (the type of x for z=mult(x,y))
// Y type:   uint64_t (the type of y for z=mult(x,y))
...
{ 
    ...
    uint64_t *restrict Sauna_Work = Sauna->Sauna_Work ;  // size C->vlen*zsize
