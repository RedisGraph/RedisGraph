//------------------------------------------------------------------------------
// GB_AxB__max_div_uint8:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__max_div_uint8
// A'*B function: GB_AdotB__max_div_uint8
...
)
{
    ...
    uint8_t *restrict w = GB_thread_local.Work ;
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    uint8_t aik = Ax [pa] ;
                    uint8_t t = flip ? IDIV(bkj,aik) : IDIV(aik,bkj) ;
                    if (flag > 0)
                    {
...
)
{
    ...
    #define MERGE                                           \
    {                                                       \
        uint8_t aki = Ax [pa++] ;    /* aki = A(k,i) */      \
        uint8_t bkj = Bx [pb++] ;    /* bjk = B(k,j) */      \
        uint8_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMAX (cij,t) ;                                   \
    ...
    for (int64_t j = 0 ; j < n ; j++)
    {
        ...
        for (int64_t kk = kk1 ; kk < kk2 ; kk++)
        {
            ...
            if (bjnz == nrows && ainz == nrows)
            {
                ...
                {
                    uint8_t aki = Ax [pa + k] ;      // aki = A(k,i)
                    uint8_t bkj = Bx [pb + k] ;      // bkj = B(k,j)
                    uint8_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;
                    cij = IMAX (cij,t) ;
                }
