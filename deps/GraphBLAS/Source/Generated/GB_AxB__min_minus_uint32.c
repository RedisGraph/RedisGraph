//------------------------------------------------------------------------------
// GB_AxB__min_minus_uint32:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__min_minus_uint32
// A'*B function: GB_AdotB__min_minus_uint32
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    uint32_t aik = Ax [pa] ;
                    uint32_t t = flip ? (bkj-aik) : (aik-bkj) ;
                    if (flag > 0)
                    {
...
void GB_AdotB__min_minus_uint32
(
    GrB_Matrix C,
    const GrB_Matrix Mask,
    const GrB_Matrix A,
...
)
{
    ...
    {                                                       \
        ...
        uint32_t t = flip ? (bkj-aki) : (aki-bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMIN (cij,t) ;                                   \
