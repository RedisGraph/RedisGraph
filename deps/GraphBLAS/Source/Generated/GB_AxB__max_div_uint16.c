//------------------------------------------------------------------------------
// GB_AxB__max_div_uint16:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__max_div_uint16
// A'*B function: GB_AdotB__max_div_uint16
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    uint16_t aik = Ax [pa] ;
                    uint16_t t = flip ? IDIV(bkj,aik) : IDIV(aik,bkj) ;
                    if (flag > 0)
                    {
...
)
{
    ...
    {                                                       \
        ...
        uint16_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMAX (cij,t) ;                                   \
