//------------------------------------------------------------------------------
// GB_AxB__min_div_int64:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__min_div_int64
// A'*B function: GB_AdotB__min_div_int64
// Z type :  int64_t (the type of C)
// XY type:  int64_t (the type of A and B)
// Identity: INT64_MAX (where cij = IMIN (cij,INT64_MAX) does not change cij)
...
)
{
    ...
    int64_t *restrict w = GB_thread_local.Work ;
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    int64_t aik = Ax [pa] ;
                    int64_t t = flip ? IDIV(bkj,aik) : IDIV(aik,bkj) ;
                    if (flag > 0)
                    {
...
)
{
    ...
    #define MERGE                                           \
    {                                                       \
        int64_t aki = Ax [pa++] ;    /* aki = A(k,i) */      \
        int64_t bkj = Bx [pb++] ;    /* bjk = B(k,j) */      \
        int64_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMIN (cij,t) ;                                   \
    ...
    for (int64_t j = 0 ; j < n ; j++)
    {
        ...
        for (int64_t kk = kk1 ; kk < kk2 ; kk++)
        {
            ...
            if (bjnz == nrows && ainz == nrows)
            {
                ...
                {
                    int64_t aki = Ax [pa + k] ;      // aki = A(k,i)
                    int64_t bkj = Bx [pb + k] ;      // bkj = B(k,j)
                    int64_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;
                    cij = IMIN (cij,t) ;
                }
