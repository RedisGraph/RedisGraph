//------------------------------------------------------------------------------
// GB_AxB__min_minus_fp32:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__min_minus_fp32
// A'*B function: GB_AdotB__min_minus_fp32
// Z type :  float (the type of C)
// XY type:  float (the type of A and B)
// Identity: INFINITY (where cij = FMIN (cij,INFINITY) does not change cij)
...
)
{
    ...
    float *restrict w = GB_thread_local.Work ;
...
)
{
    ...
    #define MERGE                                           \
    {                                                       \
        float aki = Ax [pa++] ;    /* aki = A(k,i) */      \
        float bkj = Bx [pb++] ;    /* bjk = B(k,j) */      \
        float t = flip ? (bkj-aki) : (aki-bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = FMIN (cij,t) ;                                   \
    ...
    for (int64_t j = 0 ; j < n ; j++)
    {
        ...
        for (int64_t kk = kk1 ; kk < kk2 ; kk++)
        {
            ...
            if (bjnz == nrows && ainz == nrows)
            {
                ...
                {
                    float aki = Ax [pa + k] ;      // aki = A(k,i)
                    float bkj = Bx [pb + k] ;      // bkj = B(k,j)
                    float t = flip ? (bkj-aki) : (aki-bkj) ;
                    cij = FMIN (cij,t) ;
                }
