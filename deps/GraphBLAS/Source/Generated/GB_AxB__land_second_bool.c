//------------------------------------------------------------------------------
// GB_AxB__land_second_bool:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__land_second_bool
// A'*B function: GB_AdotB__land_second_bool
// Z type :  bool (the type of C)
// XY type:  bool (the type of A and B)
...
)
{
    ...
    const bool *restrict Ax = A->x ;
    const bool *restrict Bx = B->x ;
...
void GB_AdotB__land_second_bool
(
    GrB_Matrix C,
    const GrB_Matrix Mask,
    const GrB_Matrix A,
...
)
{
    ...
    #define MERGE                                           \
    {                                                       \
        bool aki = Ax [pa++] ;    /* aki = A(k,i) */      \
        bool bkj = Bx [pb++] ;    /* bjk = B(k,j) */      \
        bool t = bkj ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
    ...
    for (int64_t j = 0 ; j < n ; j++)
    {
        ...
        for (int64_t kk = kk1 ; kk < kk2 ; kk++)
        {
            ...
            if (bjnz == nrows && ainz == nrows)
            {
                ...
                {
                    bool aki = Ax [pa + k] ;      // aki = A(k,i)
                    bool bkj = Bx [pb + k] ;      // bkj = B(k,j)
                    bool t = bkj ;
                    cij = (cij && t) ;
                }
