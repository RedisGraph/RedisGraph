//------------------------------------------------------------------------------
// GB_AxB__max_div_int8:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__max_div_int8
// A'*B function: GB_AdotB__max_div_int8
// Z type :  int8_t (the type of C)
// XY type:  int8_t (the type of A and B)
// Identity: INT8_MIN (where cij = IMAX (cij,INT8_MIN) does not change cij)
...
)
{
    ...
    int8_t *restrict w = GB_thread_local.Work ;
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    int8_t aik = Ax [pa] ;
                    int8_t t = flip ? IDIV(bkj,aik) : IDIV(aik,bkj) ;
                    if (flag > 0)
                    {
...
)
{
    ...
    {                                                       \
        ...
        int8_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMAX (cij,t) ;                                   \
