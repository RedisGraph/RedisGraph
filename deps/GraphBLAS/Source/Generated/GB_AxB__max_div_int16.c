//------------------------------------------------------------------------------
// GB_AxB__max_div_int16:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__max_div_int16
// A'*B function: GB_AdotB__max_div_int16
// Z type :  int16_t (the type of C)
// XY type:  int16_t (the type of A and B)
// Identity: INT16_MIN (where cij = IMAX (cij,INT16_MIN) does not change cij)
...
)
{
    ...
    int16_t *restrict w = GB_thread_local.Work ;

    int16_t *restrict Cx = C->x ;
    const int16_t *restrict Ax = A->x ;
    const int16_t *restrict Bx = B->x ;
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    int16_t aik = Ax [pa] ;
                    int16_t t = flip ? IDIV(bkj,aik) : IDIV(aik,bkj) ;
                    if (flag > 0)
                    {
...
)
{
    ...
    #define MERGE                                           \
    {                                                       \
        int16_t aki = Ax [pa++] ;    /* aki = A(k,i) */      \
        int16_t bkj = Bx [pb++] ;    /* bjk = B(k,j) */      \
        int16_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMAX (cij,t) ;                                   \
    ...
    int16_t *Cx = C->x ;
    const int16_t *Ax = A->x ;
    const int16_t *Bx = B->x ;
    ...
    for (int64_t j = 0 ; j < n ; j++)
    {
        ...
        for (int64_t kk = kk1 ; kk < kk2 ; kk++)
        {
            ...
            if (bjnz == nrows && ainz == nrows)
            {
                ...
                {
                    int16_t aki = Ax [pa + k] ;      // aki = A(k,i)
                    int16_t bkj = Bx [pb + k] ;      // bkj = B(k,j)
                    int16_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;
                    cij = IMAX (cij,t) ;
                }
