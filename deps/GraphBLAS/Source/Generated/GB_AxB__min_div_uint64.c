//------------------------------------------------------------------------------
// GB_AxB__min_div_uint64:  hard-coded C=A*B
//------------------------------------------------------------------------------
...
// A*B function:  GB_AxB__min_div_uint64
// A'*B function: GB_AdotB__min_div_uint64
// Z type :  uint64_t (the type of C)
// XY type:  uint64_t (the type of A and B)
// Identity: UINT64_MAX (where cij = IMIN (cij,UINT64_MAX) does not change cij)
// Multiply: t = (flip ? IDIV(bkj,aik) : IDIV(aik,bkj))
// Add:      cij = IMIN (cij,t)
...
)
{
    ...
    uint64_t *restrict w = GB_thread_local.Work ;

    uint64_t *restrict Cx = C->x ;
    const uint64_t *restrict Ax = A->x ;
    const uint64_t *restrict Bx = B->x ;
        ...
        for (int64_t j = 0 ; j < n ; j++)
        {
            ...
            for (int64_t p = Bp [j] ; p < Bp [j+1] ; p++)
            {
                ...
                for (int64_t pa = Ap [k] ; pa < Ap [k+1] ; pa++)
                {
                    ...
                    uint64_t aik = Ax [pa] ;
                    uint64_t t = flip ? IDIV(bkj,aik) : IDIV(aik,bkj) ;
                    if (flag > 0)
                    {
...
)
{
    ...
    #define MERGE                                           \
    {                                                       \
        uint64_t aki = Ax [pa++] ;    /* aki = A(k,i) */      \
        uint64_t bkj = Bx [pb++] ;    /* bjk = B(k,j) */      \
        uint64_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;                          \
        if (cij_exists)                                     \
        {                                                   \
            /* cij += A(k,i) * B(k,j) */                    \
            cij = IMIN (cij,t) ;                                   \
    ...
    for (int64_t j = 0 ; j < n ; j++)
    {
        ...
        for (int64_t kk = kk1 ; kk < kk2 ; kk++)
        {
            ...
            if (bjnz == nrows && ainz == nrows)
            {
                ...
                {
                    uint64_t aki = Ax [pa + k] ;      // aki = A(k,i)
                    uint64_t bkj = Bx [pb + k] ;      // bkj = B(k,j)
                    uint64_t t = flip ? IDIV(bkj,aki) : IDIV(aki,bkj) ;
                    cij = IMIN (cij,t) ;
                }
