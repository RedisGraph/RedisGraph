...
\begin{verbatim}
This is a BETA release.  The version has passed all tests, and is
ready to use, but it has significant new extensions to the API.
Sections 8 and 9 are new.  The default matrix format is now CSR.
This version also adds support for user threading in POSIX pthreads
and OpenMP.  I would like to get feedback on the API design for
compile-time objects and GxB_print before posting this as the
definitive Version 2.2.0.
\end{verbatim}
...
earlier), just compile with \verb'-DBYCOL', or add the following after calling
\verb'GrB_init':

    {\footnotesize
    \begin{verbatim}
    GxB_set (GxB_FORMAT, GxB_BY_COL) ; \end{verbatim}}
...
Macro                & purpose                                      \\
\hline
\verb'GxB_ABOUT'        & this particular implementation, copyright, and URL \\
\verb'GxB_DATE'         & the date of this implementation \\
\verb'GxB_SPEC'         & the GraphBLAS specification for this implementation \\
\verb'GxB_SPEC_DATE'    & the date of the GraphBLAS specification \\
\verb'GxB_LICENSE'      & the license for this particular implementation \\
\hline
\end{tabular}
}
\vspace{0.2in}
...
\verb'GrB_INSUFFICIENT_SPACE'   & output array not large enough \\
...
% According to the GraphBLAS specification, 
Any GraphBLAS method or operation can return an
...
illustrate the use of \verb'GrB_error'.  Successful GraphBLAS methods do not
modify this error message.  If a GraphBLAS method fails and then
subsequent GraphBLAS method succeeds, the error message is not modified from
the last failure.  Only a subsequent failure will cause \verb'GrB_error' to
...
GraphBLAS provides 11 built-in types and 256 built-in operators.  With these,
...
first four operators, \verb'thunk' is a pointer to a single scalar of type
\verb'int64_t', which holds the value of \verb'k'.  Each operator can be used
on any type, including user-defined types.  User-defined select operators can
also be created.
...
does this by simply returning false if all bits in the value are zero, or true
...
\verb'GrB_NULL', then the \verb'selectop' is type-generic.  The
\verb'GxB_select' operation does not pass the value of $x=a_{ij}$ to the select
function, but passes \verb'GrB_NULL' for the input \verb'x' to the user-defined
select \verb'function'.

For example, to delete all entries in a matrix equal to the additive identity
of a monoid, create a user-defined \verb'GxB_SelectOp' with a function that
compares its input value \verb'x' with the value of the identity, passed in to
it via the \verb'thunk' parameter of type \verb'void *'.  The user-defined
function should return \verb'false' if \verb'x' is equal the value
\verb'(*thunk)', and \verb'true' otherwise.
...
\verb'const' scalar value of the particular type, not a pointer.  For
...
If compiled for use in MATLAB, this maximum size is reduced to match the
MATLAB maximum index size, which is $2^{48}-1$.
...
\verb'GxB_Vector_resize' also frees the workspace kept in the vector \verb'u'
if Gustavson's method is used in \verb'GrB_mxv' or \verb'GrB_vxm' to compute
the vector \verb'u'.  It frees this space, even if the size does not change:

{\footnotesize
\begin{verbatim}
    GrB_Index nrows ;
    GrB_Vector_size (&nrows, v) ;
    GxB_Vector_resize (v, nrows) ; \end{verbatim} }

See Section~\ref{guswork} for more details.
...
\verb'GxB_Matrix_resize' also frees the workspace kept in the matrix \verb'C'
if Gustavson's method is used in \verb'GrB_mxm' to compute the matrix \verb'C'.
It frees this space, even if the size does not change:
...
\begin{verbatim}
    ...
    GxB_Matrix_resize (C, nrows, nrows) ; \end{verbatim} }
...
\begin{spec}
{\bf SPEC:} \verb'GxB_DEFAULT', \verb'GxB_AxB_METHOD', and \verb'GxB_AxB_*'
are extensions to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Desc_set' function is an extension to the spec.
\end{spec}
...
\begin{spec}
{\bf SPEC:} The \verb'GxB_Desc_get' function is an extension to the spec.
\end{spec}
...
\item \verb'GxB_set (GrB_Descriptor desc, field, value)' is another way to
    ...
    \verb'GrB_Descriptor_set', just with a generic name.  Except for
    \verb'GxB_AxB_METHOD', the descriptor settings are not hints but are always
    implemented by GraphBLAS, since they affect the results of any GraphBLAS
    operation.
...
typedef enum
{
    ...
    GxB_THREAD_SAFETY,  // thread library for thread safety
    GxB_THREADING       // current none
}
GxB_Option_Field ;
\end{verbatim} }
...
and the format of future matrices to \verb'GxB_BY_COL'. 
...
This usage is identical to \verb'GrB_Descriptor_set', just with a name that is
consistent with the other usages of this generic function.  Unlike
\verb'GrB_Descriptor_set', Future versions of \verb'GxB_Desc_set' will allow
for different types for the third parameter.  Refer to
...
\verb'GxB_MODE', \verb'GxB_THREAD_SAFETY', or \verb'GxB_THREADING'.  For
example:
...
be \verb'GrB_OUTP', \verb'GrB_MASK', \verb'GrB_INP0', \verb'GrB_INP1' or
\verb'GxB_AxB_METHOD'.  Refer to Section~\ref{desc_get} for details.
...
To set/get the global options:
    ...
    GxB_set (GxB_FORMAT, GxB_BY_COL) ;
    GxB_get (GxB_FORMAT, GxB_Format_Value *s) ; \end{verbatim} }
...
To set/get a descriptor field:
    ...
    GxB_get (GrB_Descriptor d, GrB_Desc_Field f, GrB_Desc_Value *v) ; \end{verbatim} }
...
if dot method:
    compute C<M>=A'*B
else if heap or Gustavon's method:
    A=A' ;
...
See Section~\ref{guswork} on the workspace kept in the output vector \verb'w'.
...
\newpage
%===============================================================================
\subsection{Workspace for Gustavson's method for
{\sf GrB\_mxm}, {\sf GrB\_vxm}, and {\sf GrB\_mxv} }
%===============================================================================
\label{guswork}

Gustavon's method for multiplying two sparse matrices or a sparse matrix and a
sparse vector invovles a tradeoff between workspace and time.  If a
pre-initialized workspace is kept for repeated multiplications, the time can be
significantly reduced.  This is very important for methods such as
\verb'GrB_vxm' when used in a very sparse computation such as breadth-first
search.

The workspace required by Gustavson's method is size $O(n)$ for the CSR format,
or $O(m)$ for CSC, if the output matrix \verb'C' is $m$-by-$n$, or $O(n)$ for a
vector \verb'w' of length $n$.  This space is reused for repeated used by
\verb'GrB_mxm', \verb'GrB_mxv', and \verb'GrB_vxm'.  To free this space, use
\verb'GxB_Matrix_resize' or \verb'GxB_Vector_resize', and simply keep the size
the same:
...
\begin{verbatim}
    ...
    GrB_Matrix_nrows (&nrows, C) ;
    GrB_Matrix_ncols (&ncols, C) ;
    GxB_Matrix_resize (C, nrows, nrows) ;

    GrB_Vector_size (&nrows, w) ;
    GxB_Vector_resize (w, nrows) ; \end{verbatim} }

This has no effect on the matrix \verb'C' or vector \verb'w', other than to
free the workspace kept for Gustavson's method.  If the workspace is not
allocated inside the output matrix \verb'C' or vector \verb'w', or for the the
first call to \verb'GrB_mxm(C,...)' (or \verb'GrB_vxm(w,...)'), the time taken
is $O(m+n+f)$, where $f$ is the number of numerical operations that must be
performed.

Assuming the workspace held internally in \verb'C' or \verb'w' is not freed,
subsequent uses of Gustavson's method in \verb'GrB_mxm' to compute \verb'C'
takes only $O(n+f)$ time for a CSC matrix, or $O(m+f)$ for a CSR matrix, and
subsequent uses of \verb'GrB_mxv' and \verb'GrB_vxm' with the same output
vector \verb'w' takes only $O(f)$ time.

The \verb'GxB_print' function will report if the workspace for Gustavson's
method is present, called the \verb'Sauna' (short-hand for the SParse
Accumulator, or SPA, as this workspace is called in the sparse matrix
library used internally in MATLAB \cite{GilbertMolerSchreiber92}).  See
Section~\ref{fprint}.
...
should be considered debugging tools only, not for final use in production.  To
control when these methods are used, the user application could define the
following macro and global variable \verb'pr' in a user include file:

{\footnotesize
\begin{verbatim}
#include <assert.h>
extern GxB_Print_Level pr ;
#define CHECK(object) assert (GxB_print (object, pr) == GrB_SUCCESS) \end{verbatim}}

Place the following statement:

{\footnotesize
\begin{verbatim}
GxB_Print_Level pr = GxB_SILENT ; \end{verbatim}}

\noindent in one of the source files (outside of any function), and then use
\verb'CHECK(A)' to check a matrix or any other object.  To enable printing, set
the global \verb'pr' value to a nonzero value, say \verb'GxB_SUMMARY' or
\verb'GxB_COMPLETE'.  Compile normally to enable debugging, or with
\verb'-DNDEBUG' to disable debugging when compiling the code for performance.
...
Six examples of how to use GraphBLAS are described below:
...
in the rank vector \verb'r' is below a threshold.  This is computed in one of
...
instead.  If \verb'my_pagerank.m4' is placed in \verb'GraphBLAS/User/', then
...
which appears in the \verb'SuiteSparse/GraphBLAS/Doc' folder.  That paper uses
an earlier version of SuiteSparse:GraphBLAS in which all matrices are stored by
column.
...
user threads, use the following \verb'cmake' command:
...
I would like to thank Roi Lipman, RedisLabs (\url{https://redislabs.com}), for
...
MIT Lincoln Lab and NSF grant 1514406.
