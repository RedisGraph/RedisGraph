...
\item Version 6.2.0.beta1 (Jan 26, 2022)
...
\item Versions 6.1.0 (Dec 26, 2021)
    ...
    \item bug fix: deserialization of iso and empty matrices/vectors was
        incorrect
    \end{packed_itemize}
...
\item Versions 6.0.2 and 5.2.2 (Nov 30, 2021)
    ...
    \begin{packed_itemize}
    \item (38) bug fix: \verb'GrB_Matrix_export':
...
\item Versions 6.0.1 and 5.2.1 (Nov 27, 2021)
    ...
    \item (37) bug fix:  \verb"C+=A'*B" when the accum operator is the same as
...
\item Version 5.2.0 (Nov 15, 2021)
    ...
    \item (36) bug fix: ewise \verb'C=A+B' when all matrices are full,
        \verb'GBCOMPACT' not used, but \verb'GB_control.h' disabled the
        operator or type.  Caught by Roi Lipman, Redis.
    \item (34) bug fix: \verb'C<M>=Z' not returning \verb'C'
...
\item Version 5.1.8 (Oct 5, 2021)
    ...
    \begin{packed_itemize}
    \item (31) bug fix: C=A*B when A is sparse and B is iso and bitmap.
...
\item Version 5.1.7 (Aug 23, 2021)
    ...
    \begin{packed_itemize}
    \item (30) bug fix:  \verb'GrB_apply', when done in-place and matrix starts
...
Most of the iterator methods are implemented in SuiteSparse:GraphBLAS as
\verb'static' \verb'inline' functions.  This requires that the opaque contents
of the iterator object are defined in \verb'GraphBLAS.h' itself.  The user
application must not access these contents directly, but can only do so safely
via the iterator methods provided by SuiteSparse:GraphBLAS.
...
the next sectio.  Accessing the indices is performed with four different sets
of methods, depending on which access pattern is in use, described below:
...
otherwise it works for any matrix.  If the matrix is held by-column, then use
