%TTT various tests

test53 ;  % quick test of GB_mex_Matrix_extract
test24 ;  % test of GrB_Matrix_reduce
test10 ;  % GrB_apply
test90 ;  % test pre-compiled user-defined semirings
test21b ; % quick test of GB_mex_assign
test21 ;  % quick test of GB_mex_subassign
test16 ;  % user-defined complex operators
test18 ;  % quick tests of GrB_eWiseAdd and eWiseMult
test75 ;  % test GrB_mxm A'*B on all semirings
test06 ;  % test GrB_mxm on all semirings
test19b ; % GrB_assign, many pending operators
test20 ;  % quick test of GB_mex_mxm on a few semirings

